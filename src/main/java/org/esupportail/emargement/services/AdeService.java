...
public class AdeService {
    ...
    private int processStudents(SessionEpreuve se, AdeResourceBean ade, String sessionId, List<Long> groupes, Context ctx) {
        long startTotalMs = System.currentTimeMillis();
        int nbStudents = 0;

        if ("aucune".equals(appliConfigService.getAdeImportSourceParticipants(ctx))) {
            // Pas d'import des participants
            log.info("PERF processStudents [seId={}] [skipped=source=aucune] [total={} ms]",
                    se.getId(), System.currentTimeMillis() - startTotalMs);
            return nbStudents;
        }
        ...
        if (listAdeTrainees != null && !listAdeTrainees.isEmpty()) {
            ...
            String sourceParticipants = appliConfigService.getAdeImportSourceParticipants(ctx);
            switch (sourceParticipants) {
                case "ade":
                    ...
                    long tPhase1End = System.currentTimeMillis() - tPhase1;
                    int nbTrainees = listAdeTrainees != null ? listAdeTrainees.size() : 0;
                    int nbSuperGroupes = listAdeSuperGroupes != null ? listAdeSuperGroupes.size() : 0;
                    log.info("PERF processStudents.PHASE1_membres [seId={}] [trainees={}] [superGroupes={}] [allMembers1={}] [allMembers2={}] [limitQueries={}] [total={} ms]",
                            se.getId(), nbTrainees, nbSuperGroupes, allMembers1.size(), allMembers2.size(),
                            isAdeCampusLimitQueriesEnabled, tPhase1End);
                    ...
                    if (!allMembers.isEmpty()) {
                        ...
                        if (isAdeCampusLimitQueriesEnabled) {
                            ...
                            allCodes = allMembers.stream()
                                    ...
                                    .map(list -> list.get(0))
                                    .collect(Collectors.toList());
                            phase2HttpCalls = allMembers.size();
                        }
                    }
                    ...
                    long tLdapEnd = System.currentTimeMillis() - tLdap;
                    log.info("PERF processStudents.PHASE3_ldap [seId={}] [codes={}] [filter={}] [users_found={}] [total={} ms]",
                            se.getId(), allCodes.size(), filter, users.size(), tLdapEnd);
                    long tDbLoop = System.currentTimeMillis();
                    int dbLoopFindPerson = 0, dbLoopFindTagCheck = 0, dbLoopFindAbsences = 0,
                            dbLoopSavePerson = 0, dbLoopSaveTagCheck = 0;
                    if(!allCodes.isEmpty()) {
                    ...
                    long tDbLoopEnd = System.currentTimeMillis() - tDbLoop;
                    log.info("PERF processStudents.PHASE4_dbloop [seId={}] [codes={}] [findPerson={}] [savePerson={}] [findTagCheck={}] [findAbsences={}] [saveTagCheck={}] [nbStudents={}] [total={} ms]",
                            se.getId(), allCodes.size(), dbLoopFindPerson, dbLoopSavePerson,
                            dbLoopFindTagCheck, dbLoopFindAbsences, dbLoopSaveTagCheck, nbStudents, tDbLoopEnd);
                    break;
        ...
        log.info("PERF processStudents END [seId={}] [nbStudents={}] [total={} ms]",
                se.getId(), nbStudents, System.currentTimeMillis() - startTotalMs);
        return nbStudents;
    }
    ...
    public void processLocations(SessionEpreuve se, AdeResourceBean ade, String sessionId, Context ctx,
        ...
        int nbAdeClassrooms = listAdeClassRooms != null ? listAdeClassRooms.size() : 0;
        log.info("PERF processLocations BEGIN [seId={}] [sessionId={}] [classrooms={}]",
                se.getId(), sessionId, nbAdeClassrooms);
        ...
        log.info("PERF processLocations.STEP1_load_classrooms [seId={}] [http_calls={}] [classroomsById={}] [total={} ms]",
                se.getId(), nbAdeClassrooms, classroomsById.size(), t1End);
        ...
        int slsCreated = 0, slsUpdated = 0, locsCreated = 0;
        for (Map.Entry<Long, List<AdeClassroomBean>> entry : classroomsById.entrySet()) {
        ...
        long t4End = System.currentTimeMillis() - t4;
        log.info("PERF processLocations END [seId={}] [step1_load={} ms] [step2_calc={} ms] [step3_adjust={} ms (done={})] " +
                        "[step4_persist={} ms (locsCreated={}, slsCreated={}, slsUpdated={})] [total={} ms]",
                se.getId(), t1End, t2End, t3End, adjustmentDone, t4End, locsCreated, slsCreated, slsUpdated,
                System.currentTimeMillis() - startTotalMs);
    ...
    private void processInstructors(AdeResourceBean ade, String sessionId, Context ctx, List<SessionLocation> sls) {
        ...
        int httpCalls = 0;
        int instructorsProcessed = 0;
        int ldapHits = 0, userAppCreated = 0, tagCheckersCreated = 0;
        if(listAdeInstructors!= null &&!listAdeInstructors.isEmpty()) {
            ...
            for(Map<Long, String> map : listAdeInstructors) {
                for (Entry<Long, String> entry : map.entrySet()) {
                    ...
                    httpCalls++;
                    instructorsProcessed += adeInstructorBeans.size();
        ...
        log.info("PERF processInstructors [seId={}] [http_calls={}] [instructors={}] [ldap_hits={}] [userApp_created={}] [tagCheckers_created={}] [sls={}] [total={} ms]",
                ade.getSessionEpreuve() != null ? ade.getSessionEpreuve().getId() : null,
                httpCalls, instructorsProcessed, ldapHits, userAppCreated, tagCheckersCreated, sls.size(),
                System.currentTimeMillis() - startTotalMs);
    }
